...
function isProductionValue(key, value) {
  const prodIndicators = {
    ...
    NEXT_PUBLIC_APP_URL: (v) =>
      v.includes("https://") && !v.includes("localhost"),
...
function checkEnvironment() {
  ...
  console.log(
    `${colors.bold}${colors.cyan}║        KOLLECT-IT ENVIRONMENT VARIABLE CHECKER           ║${colors.reset}`,
  );
  console.log(
    `${colors.bold}${colors.cyan}╚═══════════════════════════════════════════════════════════╝${colors.reset}\n`,
  );
  ...
  console.log(
    `  .env            : ${envDev ? colors.green + "✓ Found" : colors.red + "✗ Missing"}${colors.reset}`,
  );
  console.log(
    `  .env.production : ${envProd ? colors.green + "✓ Found" : colors.red + "✗ Missing"}${colors.reset}`,
  );
  console.log(
    `  .env.local      : ${envLocal ? colors.green + "✓ Found" : colors.yellow + "⚠ Optional"}${colors.reset}\n`,
  );
  ...
  console.log(
    `${colors.bold}Checking: ${colors.cyan}${envName}${colors.reset}\n`,
  );
  ...
  for (const [category, config] of Object.entries(REQUIRED_VARS)) {
    const criticalMarker = config.critical
    ...
    config.vars.forEach((varName) => {
      ...
      if (!value) {
        if (config.critical) {
          console.log(
            `  ${colors.red}✗${colors.reset} ${varName} ${colors.red}(MISSING - REQUIRED)${colors.reset}`,
          );
          criticalMissing++;
        } else {
          console.log(
            `  ${colors.yellow}⚠${colors.reset} ${varName} ${colors.yellow}(Missing - Optional)${colors.reset}`,
          );
          warnings++;
        }
      } else {
        ...
        if (prodCheck === false && envName === ".env.production") {
          console.log(
            `  ${colors.yellow}⚠${colors.reset} ${varName} ${colors.yellow}(Found, but looks like DEV value)${colors.reset}`,
          );
          warnings++;
        } else {
          const maskedValue =
          ...
          console.log(
            `  ${colors.green}✓${colors.reset} ${varName} ${colors.reset}(${maskedValue})`,
          );
        }
      }
    });
  ...
  console.log(
    `  Found:               ${colors.green}${foundVars}${colors.reset}`,
  );
  console.log(
    `  Missing (Critical):  ${criticalMissing > 0 ? colors.red : colors.green}${criticalMissing}${colors.reset}`,
  );
  console.log(
  ...
  if (!envProd) {
    console.log(
      `${colors.yellow}⚠ WARNING:${colors.reset} No .env.production file found!`,
    );
    console.log(
      `  Create one by copying .env and updating values for production.\n`,
    );
  }

  if (criticalMissing > 0) {
    console.log(
      `${colors.red}✗ FAILED:${colors.reset} ${criticalMissing} critical environment variable(s) missing.`,
    );
    console.log(
      `  ${colors.red}NOT READY FOR PRODUCTION DEPLOYMENT${colors.reset}\n`,
    );
    process.exit(1);
  } else if (warnings > 0) {
    console.log(
      `${colors.yellow}⚠ WARNINGS:${colors.reset} ${warnings} warning(s) detected.`,
    );
    console.log(`  Review warnings above before deploying to production.\n`);
    process.exit(0);
  } else {
    console.log(
      `${colors.green}✓ SUCCESS:${colors.reset} All required environment variables are configured!`,
    );
    console.log(
