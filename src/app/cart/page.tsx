        ...
        <div className="min-h-screen flex flex-col items-center justify-center py-16 px-4">
          <div className="max-w-md text-center">
            ...
            <p className="text-ink-light mb-8 text-lg">
              Explore our curated collection of authenticated antiques and collectibles to get started.
            </p>
            <Link href="/shop" className="inline-block bg-accent-gold text-white font-semibold px-8 py-4 rounded-lg hover:opacity-90 transition-opacity">
              Browse Collection
            </Link>
          </div>
        </div>
                ...
                {suggestions.slice(0, 6).map((p) => (
                  <Link key={p.id} href={`/product/${p.slug}`} className="group">
                    <div className="h-64 w-full overflow-hidden rounded-lg bg-surface-2 mb-4 relative">
                      {p.images[0] ? (
                        <Image 
                          src={transformCloudinary(p.images[0].url, 'thumbnail')} 
                          alt={p.title} 
                          width={300} 
                          height={300} 
                    ...
                    <p className="text-accent-gold font-semibold text-lg">{formatUSD(p.price)}</p>
      ...
      <section className="py-12 md:py-16 bg-surface-1 border-b">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <Link href="/shop" className="text-accent-gold font-semibold hover:underline">
              Continue Shopping
            </Link>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-6">
              {items.map((item) => (
                <div key={item.id} className="flex gap-6 pb-6 border-b border-surface-2">
                  {/* Product Image */}
                  <Link 
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-lg p-6 space-y-3">
                <div className="flex gap-3 text-sm">
                  ...
                  <p><strong>Secure Checkout</strong><br /><span className="text-ink-light text-xs">SSL encrypted</span></p>
                </div>
                <div className="flex gap-3 text-sm border-t border-surface-2 pt-3">
                  <span className="text-lg">✓</span>
                  <p><strong>30-Day Returns</strong><br /><span className="text-ink-light text-xs">Easy returns policy</span></p>
                </div>
                <div className="flex gap-3 text-sm border-t border-surface-2 pt-3">
                  <span className="text-lg">✓</span>
                  <p><strong>Insured Shipping</strong><br /><span className="text-ink-light text-xs">Full protection included</span></p>
                </div>
              </div>

              {/* Promo Code */}
              <div className="bg-white rounded-lg p-6">
                <label className="block text-sm font-semibold text-ink mb-2">Promo Code</label>
