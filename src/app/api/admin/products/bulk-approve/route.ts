...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const aiProduct of aiProducts) {
      try {
        const finalPrice = priceOverride ?? (useSuggestedPrices ? aiProduct.suggestedPrice : aiProduct.suggestedPrice);
        ...
        if (!categoryId) {
          throw new Error(`No matching category found for: ${aiProduct.aiCategory}`);
        }
