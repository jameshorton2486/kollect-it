...
async function sendStatusUpdateEmail(order: { orderNumber: string; customerName: string | null; customerEmail: string | null; status: string; trackingNumber: string | null; carrier: string | null }) {
  try {
    // Only send email if customer email exists
    if (!order.customerEmail) {
      console.log('No customer email, skipping status update email');
      return;
    }
