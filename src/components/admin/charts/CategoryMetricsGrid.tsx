        ...
        <tbody>
          {data.map((item: CategoryMetrics) => (
            <tr key={item.name} className="border-b border-gray-100 hover:bg-gray-50">
              ...
              <td className="py-3 px-4 text-right text-gray-700">{item.productCount}</td>
              <td className="py-3 px-4 text-right">
                <span className="text-green-600 font-medium">{item.approvalRate.toFixed(1)}%</span>
              </td>
              <td className="py-3 px-4 text-right text-gray-700">
                ${item.averagePrice.toLocaleString('en-US', { maximumFractionDigits: 0 })}
              </td>
              <td className="py-3 px-4 text-right">
