...
interface EmailTemplate {
  ...
  name: string;
  subject: string;
  type: 'order_confirmation' | 'shipping_update' | 'promotion' | 'abandoned_cart' | 'custom';
...
export function EmailNotificationManager() {
  ...
  const [selectedTemplate, _setSelectedTemplate] = useState<EmailTemplate | null>(null);
  ...
  const handleTemplateAction = async (templateId: string, action: 'activate' | 'pause' | 'delete') => {
    try {
      if (action === 'delete') {
        await fetch(`/api/admin/email/templates/${templateId}`, { method: 'DELETE' });
      } else {
  ...
  const getTemplateIcon = (type: string) => {
    switch (type) {
      case 'order_confirmation': return <Package className="w-5 h-5" />;
      case 'shipping_update': return <Send className="w-5 h-5" />;
      case 'promotion': return <DollarSign className="w-5 h-5" />;
      case 'abandoned_cart': return <AlertCircle className="w-5 h-5" />;
  ...
  return (
    <div className="space-y-6">
                  ...
                  <tr key={template.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">
                        {template.type.replace('_', ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {template.openRate !== undefined && (
                        <div className="text-sm">
                          <div className="text-gray-900">Open: {template.openRate}%</div>
                          <div className="text-gray-500">Click: {template.clickRate}%</div>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {template.lastSent ? new Date(template.lastSent).toLocaleDateString() : 'Never'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {template.status === 'active' ? (
                          <button
                            onClick={() => handleTemplateAction(template.id, 'pause')}
                          ...
                          <button
                            onClick={() => handleTemplateAction(template.id, 'activate')}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {campaigns.map((campaign) => (
                  <tr key={campaign.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="text-sm text-gray-900">{campaign.sent.toLocaleString()}</div>
      ...
      {activeTab === 'stats' && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4">Email Performance Analytics</h3>
