...
import { 
  Store, CreditCard, Truck, Calculator, 
  Tag, Save, AlertCircle, CheckCircle,
  Plus, Trash2, Edit
} from 'lucide-react';
...
export function AdminSettingsPanel() {
  ...
  const [shippingZones] = useState<ShippingZone[]>([
    { id: '1', name: 'Domestic', countries: ['US'], flatRate: 9.99, freeShippingThreshold: 100 },
    { id: '2', name: 'International', countries: ['CA', 'UK'], flatRate: 24.99, freeShippingThreshold: 200 },
  ]);
  ...
  const [categories] = useState<CategoryConfig[]>([
    { id: '1', name: 'Antiques', slug: 'antiques', description: 'Vintage and antique items', active: true },
    { id: '2', name: 'Collectibles', slug: 'collectibles', description: 'Rare collectible items', active: true },
    { id: '3', name: 'Art', slug: 'art', description: 'Fine art and prints', active: true },
  ]);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const response = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({
          tab: activeTab,
          data: activeTab === 'store' ? storeSettings :
                activeTab === 'payment' ? paymentSettings :
                activeTab === 'shipping' ? shippingZones :
                activeTab === 'tax' ? taxRates :
                categories
        }),
      });
          ...
          <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center">
                ...
                />
                <label htmlFor="stripe-enabled" className="ml-2 text-sm font-medium text-gray-700">
                  Enable Stripe Payments
                </label>
              </div>
              <div>
                <label htmlFor="stripe-publishable" className="block text-sm font-medium text-gray-700 mb-2">
                  Publishable Key
                </label>
                <input
              ...
              <div>
                <label htmlFor="stripe-secret" className="block text-sm font-medium text-gray-700 mb-2">
                  Secret Key
                </label>
                <input
                  id="stripe-secret"
            ...
            <div className="space-y-4">
              <div className="flex items-center">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="paypal-enabled" className="ml-2 text-sm font-medium text-gray-700">
                  Enable PayPal Payments
                </label>
              </div>
              <div>
                <label htmlFor="paypal-client" className="block text-sm font-medium text-gray-700 mb-2">
                  Client ID
                </label>
                <input
                  id="paypal-client"
            ...
            {shippingZones.map((zone) => (
              <div key={zone.id} className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm text-gray-600">Flat Rate</label>
                    <p className="font-medium">${zone.flatRate}</p>
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600">Free Shipping Over</label>
