    ...
    <div className="product-gallery space-y-6">
      ...
      >
        ...
        >
          ...
          <div className="absolute top-4 right-4 flex items-center gap-1.5 rounded-full bg-white/95 backdrop-blur-sm px-3 py-1.5 shadow-md border border-white/50">
            ...
            <span className="text-[11px] font-semibold text-brand-navy">Authenticated</span>
          </div>
          ...
          {/* Image Counter Badge */}
          {images.length > 1 && (
            <div className="absolute bottom-4 right-4 rounded-full bg-brand-navy/90 backdrop-blur-sm px-3 py-1.5 text-xs font-semibold text-white border border-brand-gold/30">
      ...
      {images.length > 1 && (
        <div className="product-gallery-thumbnails">
          <p className="text-xs uppercase tracking-wider text-ink-secondary font-semibold mb-3">More Views</p>
          <div className="flex gap-2 overflow-x-auto pb-2 snap-x">
            {images.map((image, index) => (
              <button
                key={index}
                className={`product-gallery-thumbnail shrink-0 snap-start rounded-lg border-2 transition-all overflow-hidden hover:border-brand-gold ${
                  index === selectedIndex 
                    ? 'border-brand-gold shadow-md ring-2 ring-brand-gold/20' 
                    : 'border-ink-tertiary/20 hover:border-brand-gold/50'
                }`}
              ...
              >
                ...
                />
                {index === selectedIndex && (
                  <div className="absolute inset-0 bg-brand-gold/10 pointer-events-none" aria-hidden="true"></div>
                )}
              </button>
            ))}
          </div>
