...
test("home renders key sections", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: /Latest Arrivals/i }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: /Shop by Category|Shop by Categories/i }),
  ).toBeVisible();
});
...
test("optional: product page exists when SMOKE_PRODUCT_SLUG provided", async ({
  page,
}) => {
  const slug = process.env.SMOKE_PRODUCT_SLUG;
  test.skip(
    !slug,
    "No SMOKE_PRODUCT_SLUG provided; skipping product page smoke.",
  );
