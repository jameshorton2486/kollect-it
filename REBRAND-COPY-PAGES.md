# 📄 REBRAND COPY - ALL PAGES

**Copy organized by page. Use this to verify all messaging before execution.**

**Status:** Ready to implement

---

## 🏠 HOMEPAGE - COMPLETE NEW COPY

### Section 1: Hero (REPLACE EVERYTHING)

**Location:** Top of page, large hero section

**Current Copy (DELETE):**

```
Discover authenticated luxury collectibles curated for discerning connoisseurs
Trusted by museums and private collectors
```

**NEW Copy:**

```
Headline: Collectibles Worth Collecting

Subheadline: Professionally valued. Fairly priced. Personally curated.

Body: We focus on the items auction houses skipâ€"the pieces worth $500-$15,000
that deserve expert attention but don't fit traditional auction minimums.

Every item in our collection is photographed from multiple angles,
accurately described, and professionally valued.

Browse our collection or contact us to sell your items.

[Button 1: Browse Collection]
[Button 2: Sell Your Items]
```

---

### Section 2: Four-Column Tagline (REPLACE)

**Location:** Below hero, horizontal section with 4 columns

**Current Copy (DELETE):**

```
Fine Art to Rare Books â€¢ Furniture to Collectibles â€¢ Expert Authentication â€¢ Insured Shipping
```

**NEW Copy:**

```
Rare Books to Fine Art â€¢ Collectibles to Militaria â€¢ Professionally Authenticated â€¢ Insured & Tracked Shipping
```

---

### Section 3: How It Works (NEW or REPLACE if exists)

**Location:** Mid-page educational section

**NEW Copy:**

```
Header: THREE STEPS TO YOUR NEXT COLLECTIBLE

Step 1 Title: Browse
Step 1 Body: Explore our collection. Detailed photos and honest descriptions for every item.

Step 2 Title: Ask Questions
Step 2 Body: Want more photos? Details? We'll respond within 24 hours.
That's the benefit of dealing directly.

Step 3 Title: Purchase with Confidence
Step 3 Body: Insured shipping. Fair pricing. Full visibility into what you're buying.
```

---

### Section 4: Authentication & Valuation (REPLACE)

**Location:** Trust section, usually mid-to-lower page

**Current Copy (DELETE):**

```
Expert-vetted items with detailed descriptions, provenance documentation,
and certification where applicable.
Bank-level encryption. Your payment information is protected by
industry-leading security standards.
```

**NEW Copy:**

```
Header: WE AUTHENTICATE. WE LISTEN. WE ADJUST IF WE'RE WRONG.

Body: Every item is professionally valued using industry-standard resources.
But we're not perfect. If you believe we've made an error, contact us.
We'll review it and adjust our assessment if needed.

Honesty is our foundation.

âœ" Professional valuations using industry-standard resources
âœ" Transparent pricing between auction and retail value
âœ" Error correction: We're open to discussion if you disagree
âœ" Third-party appraisals available for high-value items ($5K+)

Payment Security:
Your payment information is protected by industry-leading security standards.
```

---

### Section 5: Why Choose Us (REPLACE)

**Location:** Trust/credibility section

**Current Copy (DELETE):**

```
Trusted by museums and private collectors
[Any existing trust messaging]
```

**NEW Copy:**

```
Header: WHY COLLECTORS CHOOSE KOLLECT-IT

Box 1 Title: Accurate Descriptions
Box 1 Body: Every item photographed from multiple angles with honest, detailed descriptions.
Questions? We'll send more photos or clarify anything.

Box 2 Title: Fair Pricing
Box 2 Body: We price between what auctions get and what retail costs.
You save compared to traditional auction houses.

Box 3 Title: Professional Valuation
Box 3 Body: We use industry-standard resources to value items accurately.
If you disagree, we'll review it.

Box 4 Title: Real Communication
Box 4 Body: Direct contact with the owner. Real person, real answers.

Box 5 Title: Careful Shipping
Box 5 Body: Insured and tracked. Your items arrive the way they should.
```

---

### Section 6: Sell Your Items (NEW SECTION - Add or Replace)

**Location:** Mid-to-lower page, below featured products or trust section

**NEW Copy:**

```
Header: HAVE SOMETHING TO SELL?

Body: We buy and consign items from collectors like you.
Skip the auction house fees (25-50%) and sell directly.

Items $500-$15,000 welcome.

âœ" Professional evaluation
âœ" Fair offer within 48 hours
âœ" Buy or consignment options
âœ" No commission on consignment sales

[Button: Tell Us About Your Items]
```

---

### Section 7: Shipping & Returns (REPLACE if exists)

**Location:** Footer or dedicated shipping info section

**Current Copy (DELETE):**

```
White glove service and authenticated luxury collectibles
[Any existing shipping copy]
```

**NEW Copy:**

```
Header: SHIPPING

âœ" Insured and tracked shipping
âœ" Ships within 3-4 business days
âœ" Professional packaging to ensure items arrive in condition

Header: RETURNS

Body: We encourage careful review of all photos and descriptions before purchasing.
Returns are possible but discouraged. Contact us to discuss any concerns.
We want you to be confident in every purchase.
```

---

## 📂 CATEGORY PAGES (4 Pages)

### Page 1: RARE BOOKS

**Location:** /category/rare-books or /categories/rare-books

**Header:** RARE BOOKS

**Description (NEW):**

```
First editions, signed copies, historical texts, vintage finds.

We specialize in rare and valuable books that deserve professional handling.
Every book is photographed from multiple angles, condition assessed honestly,
and valued using industry-standard resources.

Price range: $200-$8,000

Featured items include:
- First editions by significant authors
- Signed copies from the author or previous owner
- Historical texts with documented provenance
- Vintage finds with interesting stories
```

---

### Page 2: FINE ART

**Location:** /category/fine-art or /categories/fine-art

**Header:** FINE ART

**Description (NEW):**

```
Paintings, prints, photography, sculptures.

We source fine art pieces that appeal to both collectors and investors.
Each piece is professionally photographed, authenticated where possible,
and valued fairly between auction house and retail prices.

Price range: $500-$15,000

Featured items include:
- Original paintings and mixed media works
- Limited-edition signed prints
- Photography prints and daguerreotypes
- Small sculptures and decorative pieces
```

---

### Page 3: COLLECTIBLES

**Location:** /category/collectibles or /categories/collectibles

**Header:** COLLECTIBLES

**Description (NEW):**

```
Vintage items, memorabilia, limited editions, sought-after finds.

Collectibles are more than nostalgia—they're pieces of cultural history
worth preserving and sharing. We curate unusual, valuable items that
have genuine collector appeal.

Price range: $300-$10,000

Featured items include:
- Vintage toys, memorabilia, and decorative items
- Limited-edition collectibles from known manufacturers
- Rare music, sports, or entertainment merchandise
- Vintage kitchenware, glassware, and home goods
```

---

### Page 4: MILITARIA

**Location:** /category/militaria or /categories/militaria

**Header:** MILITARIA

**Description (NEW):**

```
Historical uniforms, medals, patches, insignia, documents.

Military history matters. These items tell stories of sacrifice, service,
and heritage. We treat every piece with respect and professional care.

Price range: $200-$5,000

Featured items include:
- Military medals and decorations with full documentation
- Uniform items and insignia from various nations and eras
- Historical military documents and photographs
- Small arms accessories and period-correct items
```

---

## ℹ️ ABOUT PAGE (NEW - Create if doesn't exist)

**Location:** /about or /about-us

**Copy:**

```
Header: ABOUT KOLLECT-IT

Subheader: Why we started. What we do. How we're different.

---

Body Section 1:

We started with a passion for collectibles and a frustration.

Items worth $500-$15,000 fall through the cracks in the traditional market.

Auction houses have minimums around $10,000. Online marketplaces like eBay
feel risky and impersonal. Private sales are hit-or-miss. Most sellers don't
know how to value what they have. Most buyers don't know if they can trust
what they're buying.

We saw a gap.

---

Body Section 2:

We built Kollect-It to fill it.

Our collection is personally curated. Every item is accurately described,
professionally valued, and fairly priced—typically between auction value
and retail price, so you get a better deal than traditional auctions.

We're a one-person operation committed to honesty. If you have questions,
want additional photos, or disagree with our valuation, reach out.

We'll listen and adjust if we're wrong.

Because the best collectibles deserve careful attention.

---

Body Section 3:

Our Approach:

âœ" HONEST DESCRIPTIONS
Multiple angles. Condition assessed fairly. Details included.
We'll clarify anything you want to know.

âœ" FAIR VALUATIONS
We use industry-standard resources to price items between
what auctions get and what retail costs. You benefit.

âœ" PROFESSIONAL AUTHENTICATION
Where possible, we authenticate items using professional resources.
We're transparent about what we know and what we don't.

âœ" REAL COMMUNICATION
You reach me directly. No corporate email loops.
Real person, real answers.

---

Body Section 4:

For Sellers:

If you have items to sell, we're interested. We buy outright or take
items on consignment—your choice.

We offer:
âœ" Professional evaluation of your items
âœ" Fair valuations based on market research
âœ" Buy option OR consignment option
âœ" Direct payment (no auction house commissions)
âœ" Professional photography and shipping

Contact us with photos and details. We'll respond within 24-48 hours.
```

---

## 🛒 CHECKOUT / PRODUCT PAGES (Light Tweaks)

**Status:** These are probably fine. Light updates only if needed.

**Update if present:**

**Remove:**

- Any mention of "luxury," "white glove," "museums"
- "Curated for connoisseurs"
- "Prestigious collections"

**Keep:**

- Detailed product photos ✓
- Condition descriptions ✓
- Pricing ✓
- Shipping cost/timeline ✓
- Return policy ✓

**Add if not present:**

- "Ask us questions" option
- "Request more photos" link
- Clear shipping timeline (3-4 business days)

---

## 🔐 AUTHENTICATION / TRUST PAGE (if separate page exists)

**Location:** /authentication or /trust or /how-we-value

**NEW Copy:**

```
Header: HOW WE AUTHENTICATE AND VALUE ITEMS

---

Section 1:

We use a combination of professional resources, industry-standard tools,
and careful research to authenticate and value items.

But here's the honest part: We're not perfect.

If you believe we've made an error in our valuation or authentication,
we want to know. We'll review it, research further, and adjust our
assessment if needed.

Honesty is our foundation.

---

Section 2:

Our Valuation Process:

1. CONDITION ASSESSMENT
We photograph items from multiple angles and assess condition using
industry-standard terminology (Fine, Good, Fair, Poor).

2. RESEARCH
We research comparable sales, auction results, and retail pricing
to determine fair market value.

3. INDUSTRY STANDARDS
For categories like rare books, militaria, and fine art, we consult
published pricing guides and authentication resources.

4. FAIR PRICING
We price between what auction houses get (typically 60-70% of our price)
and full retail price. This is fair to both buyer and seller.

---

Section 3:

Third-Party Authentication:

For high-value items ($5,000+), we can facilitate third-party authentication
through respected industry organizations. This incurs additional cost but
provides definitive authentication.

---

Section 4:

Questions or Disagreements?

If you have questions about how we valued an item, or if you disagree
with our assessment after purchase, contact us.

We'll review your concern, research it thoroughly, and adjust if we were wrong.

[Contact Button: Contact Us]
```

---

## 💬 CALLS TO ACTION (Global Updates)

**Search and replace these across entire site:**

| Old CTA                | New CTA                           | Context              |
| ---------------------- | --------------------------------- | -------------------- |
| "Discover Luxury"      | "Browse Collectibles"             | Homepage             |
| "Explore Prestige"     | "Explore by Category"             | Navigation           |
| "Sell Your Collection" | "Sell Your Items"                 | CTA buttons          |
| "Register Now"         | "Create Account" or "Get Started" | Auth                 |
| "Add to Cart"          | "Add to Cart"                     | Keep same            |
| "Browse Collection"    | "Browse Collection"               | Keep same            |
| N/A                    | "Tell Us About Your Items"        | New seller CTA       |
| N/A                    | "Ask a Question"                  | New product page CTA |
| N/A                    | "Request More Photos"             | New product page CTA |

---

## ✅ TONE CHECK BEFORE IMPLEMENTATION

Before you update the site, verify all copy:

- [ ] No "luxury," "prestigious," "elite," "connoisseur"
- [ ] No "white glove," "museum-quality service," "curated for..."
- [ ] No overpromises ("best in the world," "guaranteed," "always")
- [ ] Honest about being one person ("I," "we," "direct contact")
- [ ] Clear about fair pricing (explain value prop)
- [ ] Open to correction ("we listen," "we adjust if wrong")
- [ ] Professional but approachable tone
- [ ] Clear CTAs for both buyers AND sellers

---

## 📋 IMPLEMENTATION ORDER (When Ready)

**Start with these highest-impact pages:**

1. **Homepage hero** (most visible)
2. **Tagline** (seen on every page)
3. **About page** (sets tone)
4. **Category pages** (4 quick updates)
5. **Sell Your Items section** (new revenue driver)
6. **Authentication page** (if separate)
7. **Footer/shipping** (reference info)
8. **CTAs** (search and replace across all pages)

---

## 🎯 SUCCESS CHECKLIST AFTER IMPLEMENTATION

- [ ] Homepage hero reads "Collectibles Worth Collecting"
- [ ] No posh/luxury language visible anywhere
- [ ] "Sell Your Items" is prominent and has clear CTA
- [ ] Category pages all have new descriptions
- [ ] About page exists and explains your story
- [ ] Mobile view is readable (text not cramped)
- [ ] Console has no new errors (F12 â†' Console)
- [ ] All links work (especially Sell Your Items contact link)
- [ ] Tone is consistent across all pages

---

## 📞 REFERENCE: KEY MESSAGES

Keep these consistent across all copy:

**Main Value Prop:**
"Professionally valued, fairly priced, personally curated"

**Why Customers Choose Us:**
"Skip the auction house fees (25-50%). Sell directly. Get fair pricing."

**For Buyers:**
"Items worth $500-$15,000 that deserve expert attention"

**For Sellers:**
"Professional evaluation, fair offers, no commission on consignment"

**Trust Signal:**
"We authenticate, listen, and adjust if we're wrong"

**Tone:**
"Real communication. One person. Genuine care."

---

**This is your complete copy roadmap. Reference this when making updates, or print it for implementation day.** ✅
