        ...
        <div className="grid gap-6 sm:grid-cols-3 sm:gap-8">
          {items.map((item) => (
            <div key={item.title} className="flex items-start gap-4">
              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-lux-gold/10 text-lux-gold">
                {item.icon}
              </div>
              <div className="space-y-1">
                <p className="text-sm font-semibold tracking-wide text-lux-gold">
                  {item.title}
                </p>
