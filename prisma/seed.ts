 ...
 * - For development: bun run db:seed
 */
...
async function main() {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "❌ SEEDING BLOCKED: Do not run database seeding in production!",
    );
  }
  ...
  const categories = [
    ...
    },
    {
      ...
      image:
        "https://www.ima-usa.com/cdn/shop/files/ONJR24HGGM025__1A.jpg?v=1731974203",
    },
  ];
  ...
  const militariaCategory = await prisma.category.findUnique({
    where: { slug: "militaria" },
  });
  ...
  if (fineArtCategory) {
    const product1 = await prisma.product.create({
      data: {
        ...
        description:
          "A stunning classical landscape oil painting from the 19th century, featuring rolling hills and pastoral scenes.",
        price: 8500,
  ...
  if (collectiblesCategory) {
    const product3 = await prisma.product.create({
      data: {
        ...
        description:
          "An exquisite Patek Philippe pocket watch in 18K gold from the 1920s.",
        price: 15800,
  ...
  if (militariaCategory) {
    const product4 = await prisma.product.create({
      data: {
        ...
        description:
          "A complete WWI British medal group, named to an individual soldier.",
        price: 1800,
