...
export default function Search() {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/products?q=${encodeURIComponent(query)}&limit=${MAX_RESULTS}`,
        );
        const data: ApiProduct[] = await res.json();
        const mapped: SearchResultItem[] = (data || [])
          .slice(0, MAX_RESULTS)
          .map((p) => ({
            id: p.id,
            title: p.title,
  ...
  return (
    <div className="relative" ref={boxRef}>
      ...
      >
        <div
          className={`flex w-full items-center gap-2 rounded border px-3 py-2 bg-surface-0 focus-within:border-gold-600 focus-within:ring-1 focus-within:ring-gold-600`}
        >
          {/* Magnifying glass */}
          <svg
            width="18"
          ...
          <input
            ...
            className="w-full bg-transparent outline-none placeholder:text-ink-secondary"
            value={query}
            onChange={(e) => {
      ...
      {open && (
        <div
          id="search-dropdown"
          className="absolute z-50 mt-2 w-[420px] max-w-[90vw] rounded border border-[hsl(var(--border-300))] bg-surface-0 p-2 shadow-card"
            ...
            <div className="p-2">
              ...
              <ul className="space-y-1">
                {recent.map((r) => (
                  <li key={r}>
                    <button
                      className="w-full rounded px-2 py-1 text-left hover:bg-surface-100"
                      onClick={() => {
                        setQuery(r);
                        setOpen(false);
              ...
              {results.map((p) => {
                ...
                return (
                  <li key={p.id}>
                    <Link
                      href={`/product/${p.slug}`}
                      className="flex items-center gap-3 rounded px-2 py-2 hover:bg-surface-100"
                      onClick={() => {
