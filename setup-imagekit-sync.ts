...
async function getGoogleCloudSetup(): Promise<{ folderId: string }> {
  ...
  log(
    "4. Create Service Account (APIs & Services > Credentials > + Create Credentials)",
    "blue",
  );
  log(
    "5. Create JSON Key and save as google-credentials.json in project root",
    "blue",
  );
...
async function getImageKitSetup(): Promise<{
  ...
  log(
    "5. Copy your URL Endpoint (https://ik.imagekit.io/yoursubdomain)",
    "blue",
  );
...
async function checkGoogleCredentialsFile(): Promise<void> {
  ...
  log("You need to download it from Google Cloud Console:", "blue");
  ...
  if (done.toLowerCase() !== "yes" && done.toLowerCase() !== "y") {
    logError(
      "Please download and save google-credentials.json before continuing",
    );
    process.exit(1);
  }
...
async function runFirstSync(): Promise<void> {
  ...
  log(
    "Now running your first sync to upload images from Google Drive to ImageKit...",
    "blue",
  );
  ...
  try {
    ...
    logWarning(
      "Sync encountered an error - this is normal if it's your first run",
    );
...
async function displayNextSteps(): Promise<void> {
  ...
  log(
    "2. Visit your ImageKit dashboard to verify images were uploaded",
    "blue",
  );
  ...
  log(
    "bun run sync-status                    # Check last sync status",
    "cyan",
  );
  log(
    "bun run sync-help                      # Show help and troubleshooting",
    "cyan",
  );
